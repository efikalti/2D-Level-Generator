...
namespace MoreMountains.TopDownEngine
{
    ...
    public class WeaponAim : MonoBehaviour
    {
        ...
        protected virtual void AutoDetectWeaponMode()
        {
            ...
            {
                if ((_weapon.Owner.LinkedInputManager.WeaponForcedMode != InputManager.WeaponForcedModes.None) && (AimControl != AimControls.Off))
                {
                    switch (_weapon.Owner.LinkedInputManager.WeaponForcedMode)
                    {
                        case InputManager.WeaponForcedModes.Off:
                            AimControl = AimControls.Off;
                            break;
                        case InputManager.WeaponForcedModes.Mouse:
                            AimControl = AimControls.Mouse;
                            break;
                        case InputManager.WeaponForcedModes.MainMovement:
                            AimControl = AimControls.PrimaryMovement;
                            break;
                        case InputManager.WeaponForcedModes.SecondaryMovement:
                            AimControl = AimControls.SecondaryMovement;
                            break;
                    }
                }

                if ((_weapon.Owner.LinkedInputManager.WeaponForcedMode == InputManager.WeaponForcedModes.None) && (_weapon.Owner.LinkedInputManager.IsMobile) && (AimControl == AimControls.Mouse))
                {
                    AimControl = AimControls.PrimaryMovement;
                }
            }
