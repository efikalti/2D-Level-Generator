...
namespace MoreMountains.TopDownEngine
{
    ...
    public class DamageOnTouch : MonoBehaviour
    {
        ...
        protected virtual void OnCollideWithDamageable(Health health)
        {
            ...
            _colliderRigidBody = health.gameObject.GetComponentNoAlloc<Rigidbody>();
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            {
                if (_gizmoSize == Vector3.zero)
                {
                    _gizmoSize = _boxCollider.bounds.size;
                    _gizmoOffset = _boxCollider.bounds.center;
                }
                Gizmos.color = _gizmosColor;
                Gizmos.matrix = Matrix4x4.TRS(this.transform.TransformPoint(_gizmoOffset), this.transform.rotation, Vector3.one);
                Gizmos.DrawCube(Vector3.zero, _gizmoSize);
            }
