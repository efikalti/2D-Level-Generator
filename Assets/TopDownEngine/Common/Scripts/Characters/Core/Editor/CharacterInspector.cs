	...
	{		
		...
		protected virtual void GenerateCharacter(Character.CharacterTypes type, Modes mode)
		{
            ...
            if (mode == Modes.TwoD)
            {
                ...
                TopDownController2D topDownController3D = (character.GetComponent<TopDownController2D>() == null) ? character.gameObject.AddComponent<TopDownController2D>() : character.GetComponent<TopDownController2D>();
                topDownController3D.Gravity = -30;                
                topDownController3D.GroundLayerMask = LayerMask.NameToLayer("Ground");
                topDownController3D.HoleLayerMask = LayerMask.NameToLayer("Hole");
			...
			if (mode == Modes.ThreeD)
            {
                ...
                topDownController3D.GroundLayerMask = LayerMask.NameToLayer("Ground");
                topDownController3D.ObstaclesLayerMask = LayerMask.NameToLayer("Obstacles");
