...
{	
	...
	{	
        ...
        /// the cinemachine controller for this level
		public CinemachineCameraController LevelCameraController { get; set; }
        #if CINEMACHINE_INSTALLED
            /// the confiner attached to the cinemachine camera
            public CinemachineConfiner Confiner { get; set; }
        #endif
        ...
        protected virtual void Start()
        {
            ...
            LevelCameraController = FindObjectOfType<CinemachineCameraController>();
            if (LevelCameraController != null)
            {

                #if CINEMACHINE_INSTALLED
                    Confiner = LevelCameraController.GetComponent<CinemachineConfiner>();
                    if (Confiner != null)
                    {
            ...
            if (LevelCameraController != null)
            {
                LevelCameraController.StartFollowing();
            }
            if (AutoFocus != null)
            {
		...
		{		
			TopDownEngineEvent.Trigger(TopDownEngineEventTypes.LevelEnd);
	    ...
	    protected virtual IEnumerator GotoLevelCo(string levelName)
		{
			...
			TopDownEngineEvent.Trigger(TopDownEngineEventTypes.UnPause);
	    ...
	    protected virtual IEnumerator SoloModeRestart()
		{
            ...
            if (GameManager.Instance.MaximumLives > 0)
            {
                ...
                {
                    TopDownEngineEvent.Trigger(TopDownEngineEventTypes.GameOver);
            ...
            TopDownEngineEvent.Trigger(TopDownEngineEventTypes.RespawnComplete);
            yield break;
		}
